...
Manually annotate cells based on prior investigation:

```{r}
ann <- read.csv("data/cell-lineage.csv", stringsAsFactors = FALSE)

sce$lineage <- ann$lineage[match(sce$position, ann$cell)]
```
...
Process EHF samples from gastrulation data:

```{r cache = FALSE}
sel <- subset(AtlasSampleMetadata, stage %in% c("E7.25", "E7.5", "E7.75", "E8.0"))

...
```{r}
deployApp(appDir = "shiny/annotated", appName = "LPMP_EHF_B", appTitle = "LPMP_EHF_B", launch.browser = FALSE, forceUpdate = TRUE)
```

Allocate more memory to the iSEE application:

```{r}
configureApp(appName = "LPMP_EHF_B", appDir = "shiny/annotated", size = "xlarge")
```
...
# Define column data variables
sce$origin <- "ATLAS"
sce$origin[sce$batch == "LPMP_EHF"] <- "LPMP_EHF"
sce$Etv2 <- counts(sce)["Etv2", ] > 0
sce$Kdr <- counts(sce)["Kdr", ] > 0
sce$Runx1 <- counts(sce)["Runx1", ] > 0
sce$Ednrb <- counts(sce)["Ednrb", ] > 0
...
origin_colours <- function(n) {
  c("LPMP_EHF" = "#4E79A7", "ATLAS" = "#BAB0AC")
}
...
```{r}
deployApp(appDir = "shiny/integrated", appName = "LPMP_EHF_B_ATLAS", appTitle = "LPMP_EHF_B_ATLAS", launch.browser = FALSE, forceUpdate = TRUE)
```

Allocate more memory to the iSEE application:

```{r}
configureApp(appName = "LPMP_EHF_B_ATLAS", appDir = "shiny/integrated", size = "xxxlarge")
```

### Secondary data

Create iSEE directory for secondary data:

```{r eval = TRUE}
library(rsconnect)
setRepositories(addURLs = c(BioC = "https://bioconductor.org/packages/3.11/bioc"))
dir.create("shiny/secondary", showWarnings = FALSE, mode = "0755")
```

Copy secondary data to iSEE directory:

```{r eval = TRUE}
file.copy("output/secondary-analysis.rds", "shiny/secondary/data.rds", overwrite = TRUE)
```

Write code to iSEE application file:

```{cat, eval = TRUE, engine.opts = list(file = "shiny/secondary/app.R"), class.source = "r"}
# Load required packages
library(iSEE)
library(SingleCellExperiment)
...
# Create iSEE object
iSEE(sce, appTitle = "LPMP_EHF_B_SECONDARY", colormap = ecm)
```

Deploy the iSEE application to the shiny server:

```{r eval = TRUE}
deployApp(appDir = "shiny/secondary", appName = "LPMP_EHF_B_SECONDARY", appTitle = "LPMP_EHF_B_SECONDARY", launch.browser = FALSE, forceUpdate = TRUE)
```

Allocate more memory to the iSEE application:

```{r eval = TRUE}
configureApp(appName = "LPMP_EHF_B_SECONDARY", appDir = "shiny/secondary", size = "xxxlarge")
```
